...
void kvm_map_pagetable(pagetable_t pgtbl) {
  // 将原来映射到全局的direct mapping 添加到每个进程所有的pgtbl中
...
// 在原来va转换为pa的基础上加一个pgtbl
uint64
kvmpa(pagetable_t pagbl, uint64 va)
{
  uint64 off = va % PGSIZE;
  pte_t *pte;
  uint64 pa;
...
int pgtblprint(pagetable_t pagetable, int depth) {
  // 一个页表有2^9 = 512个pagetable entry PTES
   for (int i = 0; i < 512; i++) {
    pte_t pte = pagetable[i];
    if (pte & PTE_V) { // 页表有效
    // 打印.. 表示深度
...
// lab3 pgtbl
// 将src页表的一部分页映射关系拷贝到dst页表中
// 只拷贝页表项，不拷贝实际的物理页内存，成功返回0,失败返回-1
int
