...
procinit(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
      ...
      // 这里注释掉了原来为所有进程分配内核栈的代码 改为在创建进程时候创建内核栈 allocproc()
...
// 新添加代码lab3
  // 加入为新进程创建独立内核页表，并将内核所需要的各种映射添加到新页表中
  p->kernelpgtbl = kvminit_newpgtbl();
  // 分配一个物理页，作为新进程的内核栈使用
  char *pa = kalloc();
  if (pa == 0)
    panic("kalloc");
  uint64 va = KSTACK((int)0);  // 将内核栈映射到固定虚拟地址上 KSTACK计算在中断下面的地址
  kvmmap(p->kernelpgtbl, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
  p->kstack = va; // 记录内核栈虚拟地址，已经是固定的了，这里记录为了减少修改其他的地方代码
// 添加代码结束
